...
//		targetEl.value= day+'/'+month+'/'+year;
	    targetEl.value = day + '-' + aryMonth[month] + '-' + year;
	    try {
	     //   alert(targetEl.onchange)
        	        targetEl.onchange();
	    }
	    catch (errr) {
...
function FnSplitDate(date1) {
    var aryDate = date1.split("-");
    var aryMonth = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
    for (var intLoop = 0; intLoop < aryMonth.length; intLoop++) {
        if (aryMonth[intLoop].toString() == aryDate[1].toString()) {
...
function days_between(date1, date2) {
    ...
    var objDateFrom = new Date(date1);
    var objDateTo = new Date(date2);

    aryDate = FnSplitDate(date1);
    objDateFrom.setFullYear(aryDate[2], aryDate[1], aryDate[0]);

    aryDate = FnSplitDate(date2);
    objDateTo.setFullYear(aryDate[2], aryDate[1], aryDate[0]);
    var date2_ms = objDateFrom.getTime()
    var date1_ms = objDateTo.getTime()
    
    
    // Calculate the difference in milliseconds
    
    var difference_ms = date1_ms - date2_ms//Math.abs()
...
function FnDateDifference(objDtmFrom, objDtmTo, objTotal) {
    if (document.getElementById(objDtmFrom).value != "" && document.getElementById(objDtmTo).value != "") {
        document.getElementById(objTotal).value = days_between(document.getElementById(objDtmFrom).value, document.getElementById(objDtmTo).value);
    }
}